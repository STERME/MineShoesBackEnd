...
public class Insertshose extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			JSONObject input = RpcHelper.readJSONObject(request);// 获取raw data 的内同
			...
			String msrp = input.getString("msrp");
			JSONObject obj = new JSONObject();
			if (connection.insertShoses(shosesid, name, category, color, vendor, description, imageUrl, price, size, width, type, msrp)) { // 插入数据库
				obj.put("status", "OK");
			} else {
